...
export default function AdminCertificates() {
  ...
  const emptyItem: Certificate = { title: '', issuer: '', date: '', expiry_date: '', credential_id: '', credential_url: '', order: 0 };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem.id) {
        await api.put(`/certificates/${editingItem.id}`, editingItem, { headers });
      } else {
        await api.post('/certificates', editingItem, { headers });
      }
    ...
    Alert.alert('تأكيد', 'هل أنت متأكد من الحذف؟', [
      { text: 'إلغاء', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            const token = await AsyncStorage.getItem('session_token');
            await api.delete(`/certificates/${id}`, { headers: { Authorization: `Bearer ${token}` } });
            fetchData();
          } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>الشهادات الاحترافية</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {items.length > 0 ? items.map((item) => (
          ...
          <View style={styles.emptyState}>
            <Ionicons name="ribbon-outline" size={64} color="#444" />
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{editingItem?.id ? 'تعديل' : 'إضافة'} شهادة</Text>
              <TouchableOpacity onPress={() => setModalVisible(false)}><Ionicons name="close" size={24} color="#fff" /></TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.inputGroup}>
                <Text style={styles.label}>اسم الشهادة *</Text>
                <TextInput style={styles.input} value={editingItem?.title || ''} onChangeText={(text) => setEditingItem(prev => prev ? {...prev, title: text} : null)} placeholder="اسم الشهادة" placeholderTextColor="#666" />
              </View>
              <View style={styles.inputGroup}>
              ...
              <View style={styles.row}>
                <View style={[styles.inputGroup, { flex: 1, marginRight: 8 }]}>
                  <Text style={styles.label}>تاريخ الحصول</Text>
                ...
                <View style={[styles.inputGroup, { flex: 1, marginLeft: 8 }]}>
                  ...
                  <TextInput style={styles.input} value={editingItem?.expiry_date || ''} onChangeText={(text) => setEditingItem(prev => prev ? {...prev, expiry_date: text} : null)} placeholder="2025" placeholderTextColor="#666" />
                </View>
              </View>
              <View style={styles.inputGroup}>
                <Text style={styles.label}>رقم الاعتماد</Text>
                <TextInput style={styles.input} value={editingItem?.credential_id || ''} onChangeText={(text) => setEditingItem(prev => prev ? {...prev, credential_id: text} : null)} placeholder="ABC123" placeholderTextColor="#666" />
              </View>
              <View style={styles.inputGroup}>
                <Text style={styles.label}>رابط التحقق</Text>
                <TextInput style={styles.input} value={editingItem?.credential_url || ''} onChangeText={(text) => setEditingItem(prev => prev ? {...prev, credential_url: text} : null)} placeholder="https://..." placeholderTextColor="#666" />
              </View>
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  certIcon: { width: 56, height: 56, borderRadius: 28, backgroundColor: 'rgba(212, 175, 55, 0.1)', justifyContent: 'center', alignItems: 'center', marginRight: 14 },
  cardContent: { flex: 1 },
